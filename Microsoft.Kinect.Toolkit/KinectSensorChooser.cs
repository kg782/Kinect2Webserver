...
namespace Microsoft.Kinect.Toolkit
{
    ...
    public class KinectSensorChooser : INotifyPropertyChanged
    {
        ...
        public string RequiredConnectionId
        {
            ...
            set
            {
                ...
                using (var callbackLock = new CallbackLock(lockObject))
                {
                    ...
                    if (this.Kinect != null && this.Kinect.UniqueKinectId == requiredConnectionId)
                    {
        ...
        private void KinectSensorsOnStatusChanged(object sender, IsAvailableChangedEventArgs e)
        {
            if (e != null)
            {
                using (var callbackLock = new CallbackLock(lockObject)) {
                    if (e.IsAvailable)
                    {
                        SetSensorAndStatus(callbackLock, this.Kinect, ChooserStatus.SensorStarted);
                    }
                    else
                    {
                        SetSensorAndStatus(callbackLock, this.Kinect, ChooserStatus.NoAvailableSensors);
                    }
                }
        ...
        private void TryFindAndStartKinect(CallbackLock callbackLock)
        {
            ...
            if (Kinect != null && Kinect.IsAvailable)
            {
                ...
                if (Kinect.UniqueKinectId == requiredConnectionId)
                {
            ...
            KinectSensor newSensor = KinectSensor.GetDefault();
            newSensor.IsAvailableChanged += KinectSensorsOnStatusChanged;
            ...
            else if (!newSensor.IsAvailable)
            {
                newStatus |= ChooserStatus.SensorInitializing;
            }
            else
            {
