...
namespace Microsoft.Samples.Kinect.Webserver
{
    ...
    public static class JsonSerializationExtensions
    {
        ...
        public static object ExtractSerializableJsonData(object value)
        {
            ...
            if (IsDictionary(value))
            {
                ...
                foreach (var key in dict.Keys)
                {
                    ...
                    result.Add(k, ExtractSerializableJsonData(dict[key]));
                }
            ...
            else
            {
                ...
                // Apparently, new Kinect SDK provides past properties as fields
                if (
                    typeName == "Joint" ||
                    typeName == "JointOrientation" ||
                    typeName == "Vector4" ||
                    typeName == "CameraSpacePoint")
                {
                    foreach (var f in value.GetType().GetFields())
                    {
                        result.Add(ToCamelCase(f.Name), ExtractSerializableJsonData(f.GetValue(value)));
                    }
                }
                else
                {
