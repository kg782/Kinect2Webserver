...
namespace Microsoft.Samples.Kinect.Webserver.Sensor
{
    ...
    public class DefaultUserStateManager : IUserStateManager
    {
        ...
        /// </summary>
        /// <param name="frameBodies">
        /// Array of bodies from which the appropriate user tracking Ids will be selected.
        /// </param>
        /// <param name="timestamp">
        /// Timestamp from body frame.
        /// </param>
        ...
        /// </param>
        public void ChooseTrackedUsers(Body[] frameBodies, long timestamp, int[] chosenTrackingIds)
        {
            if (frameBodies == null)
            {
                throw new ArgumentNullException("frameBodies");
            }
            ...
            var availableBodies = new List<Body>(
                from body in frameBodies
                where
                    (body.TrackingId != SharedConstants.InvalidUserTrackingId)
                    &&
                    ((body.TrackingState == BodyTrackingState.Tracked)
                     || (body.TrackingState == BodyTrackingState.PositionOnly))
                select body);
            var trackingCandidateBodies = new List<Body>();

            // Update user activity metrics
            this.activityMeter.Update(availableBodies, timestamp);

            foreach (var body in availableBodies)
            {
                UserActivityRecord record;
                if (this.activityMeter.TryGetActivityRecord(body.TrackingId, out record))
                {
                    // The tracked bodies become candidate bodies for tracking if we have an activity record for them.
                    trackingCandidateBodies.Add(body);
                }
            }

            // sort the currently tracked bodies according to our tracking choice criteria
            trackingCandidateBodies.Sort((left, right) => this.ComputeTrackingMetric(right).CompareTo(this.ComputeTrackingMetric(left)));

            for (int i = 0; i < chosenTrackingIds.Length; ++i)
            {
                chosenTrackingIds[i] = (i < trackingCandidateBodies.Count) ? trackingCandidateBodies[i].TrackingId : SharedConstants.InvalidUserTrackingId;
            }
        }
        ...
        public void UpdateUserInformation(IEnumerable<UserInfo> trackedUserInfo, long timestamp)
        {
            ...
            using (var callbackLock = new CallbackLock(this.lockObject))
            {
                ...
                {
                    if (userInfo.BodyTrackingId == SharedConstants.InvalidUserTrackingId)
                    {
                        continue;
                    }

                    if (this.EngagedUserTrackingId == userInfo.BodyTrackingId)
                    {
                        this.TrackedUserTrackingIds.Add(userInfo.BodyTrackingId);

                        foundEngagedUser = true;
                    }
                    else if (HasTrackedHands(userInfo)
                             && (this.previousTrackedUserTrackingIds.Contains(userInfo.BodyTrackingId)
                                 || this.IsInactive(userInfo, timestamp)))
                    {
                        ...
                        if (firstTrackedUser == SharedConstants.InvalidUserTrackingId)
                        {
                            ...
                            firstTrackedUser = userInfo.BodyTrackingId;
                        }
                    }
                }
        ...
        /// </param>
        /// <returns>
        /// Body point, if last position is being tracked for specified
        /// tracking Id, null otherwise.
        /// </returns>
        public CameraSpacePoint? TryGetLastPositionForId(int trackingId)
        {
        ...
        private void UpdatePrimaryUser(IEnumerable<UserInfo> candidateUserInfo, CallbackLock callbackLock)
        {
            ...
            foreach (var userInfo in candidateUserInfo)
            {
                ...
                trackingIdsAvailable.Add(userInfo.BodyTrackingId);

                foreach (var handPointer in userInfo.HandPointers)
                {
                    if (handPointer.IsPrimaryForUser)
                    {
                        if (this.PrimaryUserTrackingId == userInfo.BodyTrackingId)
                        {
                        ...
                        else if (SharedConstants.InvalidUserTrackingId == firstPrimaryUserCandidate)
                        {
                            ...
                            firstPrimaryUserCandidate = userInfo.BodyTrackingId;
                        }
        ...
        /// <summary>
        /// Calculate how valuable it will be to keep tracking the specified body.
        /// </summary>
        /// <param name="body">
        /// Body that is one of several candidates for tracking.
        /// </param>
        /// <returns>
        /// A non-negative metric that estimates how valuable it is to keep tracking
        /// the specified body. The higher the value, the more valuable the body
        /// is estimated to be.
        /// </returns>
        private double ComputeTrackingMetric(Body body)
        {
            ...
            double engagedMetric = (body.TrackingId == this.EngagedUserTrackingId) ? 1.0 : 0.0;
            double trackedMetric = this.TrackedUserTrackingIds.Contains(body.TrackingId) ? 1.0 : 0.0;
            double angleFromCenterMetric = (body.Position.Z > 0.0) ? (1.0 - Math.Abs(2 * Math.Atan(body.Position.X / body.Position.Z) / Math.PI)) : 0.0;
            double distanceFromCameraMetric = (MaxCameraDistance - body.Position.Z) / MaxCameraDistance;
            UserActivityRecord activityRecord;
            double bodyMovementMetric = this.activityMeter.TryGetActivityRecord(body.TrackingId, out activityRecord)
        ...
        {
            UserActivityRecord record;
            return this.activityMeter.TryGetActivityRecord(userInfo.BodyTrackingId, out record) && !record.IsActive
