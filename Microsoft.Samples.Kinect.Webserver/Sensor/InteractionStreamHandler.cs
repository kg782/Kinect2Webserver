...
namespace Microsoft.Samples.Kinect.Webserver.Sensor
{
    ...
    public class InteractionStreamHandler : SensorStreamHandlerBase, IInteractionClient
    {
        ...
        public override void OnSensorChanged(KinectSensor newSensor)
        {
            if (this.sensor != null)
            {
                try
                {
                    ...
                    this.sensor.BodyStream.AppChoosesBodies = false;
                }
            ...
            if (newSensor != null)
            {
                try
                {
                    ...
                    this.sensor.BodyStream.AppChoosesBodies = true;
        ...
        /// <see cref="DepthFrame"/> from which we obtained depth data.
        /// </param>
        public override void ProcessDepth(Buffer depthData, DepthFrame depthFrame)
        {
            if (depthData == null)
            {
        ...
        public override void ProcessBody(Body[] bodies, BodyFrame bodyFrame)
        {
            ...
            if (this.ShouldProcessInteractionData)
            {
                this.interactionStream.ProcessBody(bodies, this.sensor.AccelerometerGetCurrentReading(), bodyFrame.Timestamp);
            }

            this.userStateManager.ChooseTrackedUsers(bodies, bodyFrame.Timestamp, this.recommendedUserTrackingIds);

            try
            {
                this.sensor.BodyStream.ChooseBodies(
                    this.recommendedUserTrackingIds[0], this.recommendedUserTrackingIds[1]);
            }
            catch (InvalidOperationException)
            {
                // KinectSensor might enter an invalid state while choosing bodies.
                // E.g.: sensor might be abruptly unplugged.
            }
        }
        ...
        /// </summary>
        /// <param name="bodyTrackingId">
        /// The body tracking ID for which interaction information is being retrieved.
        /// </param>
        /// <param name="handType">
        ...
        /// </returns>
        public InteractionInfo GetInteractionInfoAtLocation(int bodyTrackingId, InteractionHandType handType, double x, double y)
        {
            ...
            {
                var result = this.clientRpcChannel.CallFunction<InteractionStreamHitTestInfo>("getInteractionInfoAtLocation", bodyTrackingId, handType.ToString(), x, y);
                if (result.Success)
                {
        ...
        /// <see cref="DepthFrame"/> from which we obtained depth data.
        /// </param>
        private async void ProcessUserViewerImageAsync(Buffer depthData, DepthFrame depthFrame)
        {
