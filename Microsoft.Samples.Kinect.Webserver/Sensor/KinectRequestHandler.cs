...
namespace Microsoft.Samples.Kinect.Webserver.Sensor
{
    ...
    public class KinectRequestHandler : IHttpRequestHandler
    {
        ...
        /// <summary>
        /// Intermediate storage for the body data received from the Kinect sensor.
        /// </summary>
        private Body[] bodies;
        ...
        private void OnKinectChanged(KinectSensor newSensor)
        {
            if (this.sensor != null)
            {
                try
                {
                    ...
                    this.depthFrameReader.FrameArrived -= this.SensorDepthFrameReady;
                    this.bodyFrameReader.FrameArrived -= this.SensorbodyFrameReady;

                    //this.sensor.DepthStream.Range = DepthRange.Default;
                    //this.sensor.BodyStream.EnableTrackingInNearRange = false;
                    //this.sensor.DepthStream.Disable();
                    //this.sensor.BodyStream.Disable();
                }
            ...
            if (newSensor != null)
            {
                // Allocate space to put the body and interaction data we'll receive
                this.sensor = newSensor;

                try
                {
                    this.colorFrameReader = newSensor.ColorFrameSource.OpenReader();
                    this.depthFrameReader = newSensor.DepthFrameSource.OpenReader();
                    this.bodyFrameReader = newSensor.BodyFrameSource.OpenReader();

                    FrameDescription colorFrameDescription = newSensor.ColorFrameSource.CreateFrameDescription(ColorImageFormat.Rgba);
                    ...
                    //newSensor.BodyStream.Enable();

                    //try
                    //{
                    //    newSensor.DepthStream.Range = DepthRange.Near;
                    //    newSensor.BodyStream.EnableTrackingInNearRange = true;
                    //}
                    //catch (InvalidOperationException)
                    //{
                    //    // Non Kinect for Windows devices do not support Near mode, so reset back to default mode.
                    //    newSensor.DepthStream.Range = DepthRange.Default;
                    //    newSensor.BodyStream.EnableTrackingInNearRange = false;
                    //}

                    //this.bodies = new Body[newSensor.BodyStream.FrameBodyArrayLength];

                    //newSensor.ColorFrameReady += this.SensorColorFrameReady;
                    //newSensor.DepthFrameReady += this.SensorDepthFrameReady;
                    //newSensor.bodyFrameReady += this.SensorbodyFrameReady;
                }
                catch (InvalidOperationException)
                {
                    // KinectSensor might enter an invalid state while enabling/disabling streams or stream features.
        ...
        private void SensorColorFrameReady(object sender, ColorFrameArrivedEventArgs colorImageFrameReadyEventArgs)
        {
            ...
            if (this.colorFrameReader != sender)
            {
                return;
            }

            using (ColorFrame colorFrame = colorImageFrameReadyEventArgs.FrameReference.AcquireFrame())
            {
                if (null != colorFrame)
                {
                    try
                    {
                        ...
                        foreach (var handler in this.streamHandlers)
                        {
                            handler.ProcessColor(colorPixels, colorFrame);
                        }
                    }
                    catch (InvalidOperationException)
                    {
        ...
        private void SensorDepthFrameReady(object sender, DepthFrameArrivedEventArgs depthImageFrameReadyEventArgs)
        {
            // Even though we un-register all our event handlers when the sensor
            // changes, there may still be an event for the old sensor in the queue
            // due to the way the KinectSensor delivers events.  So check again here.
            if (this.depthFrameReader != sender)
            {
                return;
            }

            using (DepthFrame depthFrame = depthImageFrameReadyEventArgs.FrameReference.AcquireFrame())
            {
                if (null != depthFrame)
                {
                    try
        ...
        /// <summary>
        /// Handler for the Kinect sensor's bodyFrameReady event
        /// </summary>
        /// <param name="sender">object sending the event</param>
        /// <param name="bodyFrameReadyEventArgs">event arguments</param>
        private void SensorbodyFrameReady(object sender, BodyFrameArrivedEventArgs bodyFrameReadyEventArgs)
        {
            // Even though we un-register all our event handlers when the sensor
            // changes, there may still be an event for the old sensor in the queue
            // due to the way the KinectSensor delivers events.  So check again here.
            if (this.bodyFrameReader != sender)
            {
                return;
            }

            using (BodyFrame bodyFrame = bodyFrameReadyEventArgs.FrameReference.AcquireFrame())
            {
                if (null != bodyFrame)
                {
                    try
                    {
                        // Copy the body data from the frame to an array used for temporary storage
                        ...
                        foreach (var handler in this.streamHandlers)
                        {
                            handler.ProcessBody(this.bodies, bodyFrame);
                        }
                    }
                    catch (InvalidOperationException)
                    {
                        // BodyFrame functions may throw when the sensor gets
                        // into a bad state.  Ignore the frame in that case.
                    }
                }
            }
